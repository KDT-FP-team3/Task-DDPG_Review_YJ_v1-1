...
| 요소 | 기호 | 설명 |
|------|------|------|
| 상태 | s ∈ S | 환경의 현재 상태 (측정값) |
| 행동 | a ∈ A | 에이전트의 제어 신호 |
| 보상 | r | 행동에 대한 즉각적 피드백 |
| 정책 | π: S → A | 상태 → 행동 매핑 함수 |
| 할인율 | γ ∈ [0,1] | 미래 보상의 현재 가치 |
...
**누적 할인 보상 (Return):**
R_t = Σ r(s_i, a_i) * γ^(i-t) (i=t to T)

**목표:** 기대 누적 보상 최대화
J = E[R_1] (Expectation over trajectories)

상태 s_t에서 행동 a_t를 취하고 이후 정책 π를 따를 때의 기대 누적 보상:

Q^π(s_t, a_t) = E[R_t | s_t, a_t]

**벨만 방정식 (Bellman Equation):**
Q^π(s_t, a_t) = E[ r(s_t, a_t) + γ * E[Q^π(s_{t+1}, a_{t+1})] ]

결정론적 정책 μ: S → A의 경우:
Q^μ(s_t, a_t) = E[ r(s_t, a_t) + γ * Q^μ(s_{t+1}, μ(s_{t+1})) ]
...
**Actor 업데이트 (정책 경사):**
∇_θμ J ≈ E[ ∇_a Q(s, a | θ^Q) * ∇_θμ μ(s | θ^μ) ]

**Critic 업데이트 (TD 오차 최소화):**
L(θ^Q) = E[ (y_i - Q(s_i, a_i | θ^Q))^2 ]
...
[에피소드 루프] for episode = 1 to M:
  ...
  [시간 스텝 루프] for t = 1 to T:
   ...
   12. Actor 업데이트 (샘플 정책 경사):
       ∇_θμ J ≈ (1/N) * Σ ∇_a Q(s,a|θ^Q) * ∇_θμ μ(s|θ^μ)
...
**적분 오차 상태 도입:**
[ x_p_dot ] = [ Ap  0 ] [ x_p ] + [ Bp ] u + [ 0 ] r
[ e_I_dot ]   [ -Cp 0 ] [ e_I ]   [ 0  ]     [ 1 ]

**이차 비용 함수:**
J = (1/2) * ∫ (x^T * Q_LQR * x + u_c^T * R_LQR * u_c) dt

**최적 제어 신호:**
u_c(t) = -K_LQR * x_p(t) + K_I * e_I(t)

### 5.3 DDPG 제어기 변형

**DDPG₁:** Critic 네트워크가 상태 피드백 이득 K_DDPG 추정
a = u_c = K1*x1 + K2*x2 + K3*x3 = K_DDPG * x
...
**공통 관측값:**
s = x = [x1, x2, x3]^T = [x_p, e_I]^T

**보상 함수 (LQR 비용 함수의 음수):**
r(t) = -(x(t)^T * Q_LQR * x(t) + u_c^T * R_LQR * u_c)
...
| C4. 정착 시간 | t_s | 최종값 ±e_ss 범위 내 유지 시작 시간 | 작을수록 좋음 |
| C5. 정상상태 오차 | e_ss = r - y(t_f) | 원하는 출력과 실제 출력의 차이 | 0에 가까울수록 좋음 |
